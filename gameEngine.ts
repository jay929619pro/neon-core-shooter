...
export class GameEngine {
  ...
  public applyUpgrade(upgrade: UpgradeOption) {
    ...
    switch (upgrade.type) {
      case UpgradeType.FIRE_RATE: this.player.fireRate = Math.max(40, this.player.fireRate - 15); break;
      case UpgradeType.SIDE_GUNS: this.player.sideGuns++; break;
      case UpgradeType.BIG_BULLET: this.player.damageAdditive += 0.4; this.player.bulletScale += 0.2; break;
      case UpgradeType.CANNON: this.player.damageAdditive += 0.6; break;
      case UpgradeType.RANGE_BOOST: this.player.bulletScale += 0.1; break;
      case UpgradeType.VOLT_SHOT: this.player.damageAdditive += 0.2; break;
    ...
    if (cannonLevel >= 5 && rangeLevel >= 1 && this.player.weaponMode !== WeaponMode.BLACK_HOLE) {
      ...
      this.shake(60);
      for(let i=0; i<30; i++) this.spawnParticle(this.player.x, this.player.y, (Math.random()-0.5)*15, (Math.random()-0.5)*15, 8, '#bf00ff', 40, true);
    }
  }
  ...
  public reset() {
    this.player = {
      x: CANVAS_WIDTH / 2, y: CANVAS_HEIGHT * 0.8, radius: 20,
      targetX: CANVAS_WIDTH / 2, targetY: CANVAS_HEIGHT * 0.8,
      fireRate: INITIAL_FIRE_RATE, lastFired: 0, sideGuns: 0,
      bulletScale: 1, damageBase: INITIAL_DAMAGE, damageAdditive: 0, damageMultiplicative: 1,
      weaponMode: WeaponMode.STANDARD,
      maxHearts: 3, currentHealth: 6, invincibleTimer: 0,
      heartScaleAnims: [1, 1, 1]
    };
    this.bullets = []; this.enemyBullets = []; this.enemies = []; this.obstacles = [];
    this.gravityFields = []; this.particles = []; this.gems = []; this.damageNumbers = [];
    this.hearts = [];
    this.upgradeLevels.clear(); this.tagCounts.clear();
    this.score = 0; this.level = 1; this.exp = 0; this.maxExp = BalanceController.getXPRequired(1);
    this.shakeTimer = 0; this.isPaused = false; this.bossActive = false;
    this.lastKillFrame = this.frameCount; this.mercyBoost = 0;
    if (this.animationId) cancelAnimationFrame(this.animationId);
  }

  public start() { this.isPaused = false; this.loop(); }
  public resume() { if (this.isPaused) { this.isPaused = false; this.loop(); } }
  ...
  private update() {
    ...
    for (let i = this.gravityFields.length - 1; i >= 0; i--) {
      ...
      f.y += 1.2;
      const dx = f.x - this.player.x; const dy = f.y - this.player.y;
    ...
    if (Date.now() - this.player.lastFired > this.player.fireRate) {
      this.fire(); this.player.lastFired = Date.now();
    }
    ...
    for (let i = this.obstacles.length - 1; i >= 0; i--) {
      ...
      if (o.hp <= 0) {
        this.score += 50; eventBus.emit(EVENTS.TRIGGER_SOUND, 'explosion');
        for(let j=0; j<15; j++) this.spawnParticle(o.x+o.width/2, o.y+o.height/2, (Math.random()-0.5)*10, (Math.random()-0.5)*10, 4, COLORS.OBSTACLE, 30);
    ...
    for (let i = this.bullets.length - 1; i >= 0; i--) {
      ...
      if (b.isBlackHole) {
        ...
        this.enemies.forEach(e => {
          const dx = b.x - e.x; const dy = b.y - e.y; const distSq = dx*dx + dy*dy;
          if (distSq < bhConf.pullRadius! * bhConf.pullRadius!) { 
            const d = Math.sqrt(distSq) || 1;
            e.x += (dx/d) * bhConf.pullStrength!; e.y += (dy/d) * bhConf.pullStrength!; 
          }
        });
      }
    ...
    for (let i = this.enemies.length - 1; i >= 0; i--) {
      ...
      switch(e.type) {
        ...
        case EnemyType.SHOOTER:
          if (e.y < 180) e.y += e.speed; else e.x += Math.sin(this.frameCount * 0.02) * 2.5;
          ...
          break;
        case EnemyType.BOSS:
          e.x += Math.sin(this.frameCount * 0.035) * 6; e.y = Math.min(e.y + 0.8, 140);
          e.cooldown = (e.cooldown || 0) - 1;
          if (e.cooldown <= 0) { this.bossAttack(e); e.cooldown = arch.shootInterval || 100; }
          break;
        default: e.y += e.speed;
      }
      ...
      for (let bi = this.bullets.length - 1; bi >= 0; bi--) {
        ...
        if (hitDistSq < combinedRadius * combinedRadius) {
          ...
          e.hp -= finalDamage; e.hitFlash = 1; e.shakeX = (Math.random()-0.5)*15;
          ...
          if (e.hp <= 0) { this.handleEnemyDeath(e, i); break; }
        }
      }
    ...
    for (let i = this.hearts.length - 1; i >= 0; i--) {
      ...
      const dx = this.player.x - h.x; const dy = this.player.y - h.y;
      const dsq = dx*dx + dy*dy;
      if (dsq < 220*220) {
        const d = Math.sqrt(dsq) || 1;
        h.x += (dx/d)*10; h.y += (dy/d)*10;
      }
    ...
    for (let i = this.gems.length - 1; i >= 0; i--) {
      const g = this.gems[i];
      const dx = this.player.x - g.x; const dy = this.player.y - g.y;
      if (dx*dx + dy*dy < 220*220) { 
        const d = Math.sqrt(dx*dx + dy*dy) || 1;
        g.x += (dx/d)*16; g.y += (dy/d)*16; 
      }
      if ((dx*dx + dy*dy) < (this.player.radius + g.size)**2) {
        this.exp += g.value; this.gems.splice(i, 1); 
    ...
    for (let i = this.damageNumbers.length - 1; i >= 0; i--) {
      ...
      dn.x += dn.vx; dn.y += dn.vy; dn.vy += 0.3; dn.life--;
  ...
  private spawnDamageNumber(x: number, y: number, damage: number, isCrit: boolean) {
    this.damageNumbers.push({
      ...
      vx: (Math.random() - 0.5) * 5, vy: -6 - Math.random() * 5,
      text: BalanceController.formatValue(damage),
      color: isCrit ? '#ff0000' : (damage > 0 ? '#ffff00' : '#ffffff'),
      life: 45, maxLife: 45, isCrit
    });
    if (isCrit) this.shake(8);
  }

  private gameOver() { this.isPaused = true; eventBus.emit(EVENTS.GAME_OVER); }

  private levelUp() {
    this.isPaused = true; this.level++; this.exp = 0; 
  ...
  private handleEnemyDeath(e: Enemy, index: number) {
    ...
    if (e.type === EnemyType.SPLITTER) {
      ...
      for (let j = 0; j < 2; j++) {
        this.enemies.push({
          type: EnemyType.BASIC, x: e.x + (j === 0 ? -25 : 25), y: e.y,
          hp: subHp, maxHp: subHp, size: 24, color: COLORS.ENEMIES.BASIC,
          shakeX: 0, shakeY: 0, hitFlash: 0, speed: 4
        });
      }
    }

    if (e.type === EnemyType.BOSS) {
      this.bossActive = false; this.score += 2000;
      for(let k=0; k<15; k++) this.gems.push({ x: e.x + (Math.random()-0.5)*180, y: e.y + (Math.random()-0.5)*180, value: 70, size: 10 });
      // BOSS 必掉血包
      this.hearts.push({ x: e.x, y: e.y, size: 12, pulse: 0 });
    } else {
      this.score += 25; this.gems.push({ x: e.x, y: e.y, value: 30, size: 9 });
      // 普通怪低概率掉血包
      if (Math.random() < 0.05) {
        this.hearts.push({ x: e.x, y: e.y, size: 12, pulse: 0 });
      }
    ...
    this.explodeEnemy(e); this.enemies.splice(index, 1);
  ...
  private bossAttack(boss: Enemy) {
    ...
    if (pattern === 0) {
      ...
      const dx = this.player.x - boss.x; const dy = this.player.y - boss.y; const d = Math.sqrt(dx*dx + dy*dy) || 1;
  ...
  private fire() {
    ...
    const createB = (ox: number, isS = false) => {
      ...
      this.bullets.push({
        x: this.player.x+ox, y: this.player.y - (mode === WeaponMode.BLACK_HOLE ? 40 : 20), 
        vx: 0, vy: conf.bulletSpeed,
        ...
        trail: [], isBlackHole: mode === WeaponMode.BLACK_HOLE
      });
    };
    createB(0);
    for (let i = 1; i <= this.player.sideGuns; i++) { createB(-35*i, true); createB(35*i, true); }
  }
  ...
  private spawnEnemy() {
    ...
    this.enemies.push({
      type: t, x: arch.size + Math.random()*(CANVAS_WIDTH-arch.size*2), y: -arch.size,
      hp, maxHp: hp, size: arch.size, color: arch.color,
      shakeX: 0, shakeY: 0, hitFlash: 0, speed: arch.speedBase + Math.random()*2, cooldown: 0
    });
  }

  private spawnBoss() {
    this.bossActive = true; const arch = ENEMY_ARCHETYPES[EnemyType.BOSS]; const hp = BalanceController.getEnemyHP(EnemyType.BOSS, this.level);
    this.enemies.push({
      type: EnemyType.BOSS, x: CANVAS_WIDTH/2, y: -150, hp, maxHp: hp, size: arch.size, color: arch.color,
      shakeX: 0, shakeY: 0, hitFlash: 0, speed: arch.speedBase, cooldown: 60
    });
  }

  private spawnEnvironment() {
    if (Math.random() > 0.5) {
      const conf = ENVIRONMENT_CONFIG.obstacle; const hp = conf.hp * (1 + this.level/10);
  ...
  private createExplosion(x: number, y: number, r: number, d: number) {
    this.enemies.forEach(e => { if ((e.x-x)**2 + (e.y-y)**2 < r*r) { e.hp -= d * 0.8; e.hitFlash = 1; } });
    for(let i=0; i<12; i++) this.spawnParticle(x, y, (Math.random()-0.5)*22, (Math.random()-0.5)*22, 6, COLORS.BLAST, 20, true);
  }
  ...
  private chainLightning(source: Enemy, d: number, range: number) {
    ...
    if (target) {
      target.hp -= d * 1.1; target.hitFlash = 1;
  ...
  private explodeEnemy(e: Enemy) {
    ...
    for (let i = 0; i < c; i++) this.spawnParticle(e.x, e.y, (Math.random()-0.5)*20, (Math.random()-0.5)*20, Math.random()*8+2, e.color, 40, true);
  }

  private shake(i: number) { this.shakeTimer = Math.max(this.shakeTimer, i); eventBus.emit(EVENTS.SHAKE_SCREEN, i); }
  ...
  private draw() {
    ...
    this.ctx.fillStyle = COLORS.BACKGROUND; this.ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
    ...
    this.gravityFields.forEach(f => {
      ...
      grad.addColorStop(0, COLORS.GRAVITY); grad.addColorStop(1, 'transparent');
      this.ctx.fillStyle = grad; this.ctx.beginPath(); this.ctx.arc(f.x, f.y, f.radius + pulse, 0, Math.PI * 2); this.ctx.fill();
    });

    this.obstacles.forEach(o => {
      this.ctx.fillStyle = COLORS.OBSTACLE; this.ctx.strokeStyle = 'rgba(255,255,255,0.2)'; this.ctx.lineWidth = 2;
      this.ctx.strokeRect(o.x, o.y, o.width, o.height); this.ctx.fillRect(o.x, o.y, o.width, o.height);
      this.ctx.beginPath(); this.ctx.moveTo(o.x, o.y + o.height/2); this.ctx.lineTo(o.x + o.width, o.y + o.height/2); this.ctx.stroke();
      const hpR = o.hp / o.maxHp;
      this.ctx.fillStyle = 'rgba(255,255,255,0.1)'; this.ctx.fillRect(o.x, o.y - 10, o.width, 4);
      this.ctx.fillStyle = '#ffaa00'; this.ctx.fillRect(o.x, o.y - 10, o.width * hpR, 4);
    });
    ...
    this.gems.forEach(g => {
      this.ctx.fillStyle = COLORS.EXP; this.ctx.beginPath(); this.ctx.arc(g.x, g.y, g.size, 0, Math.PI * 2); this.ctx.fill();
    ...
    this.enemyBullets.forEach(eb => {
      this.ctx.fillStyle = COLORS.ENEMY_BULLET; this.ctx.shadowBlur = 10; this.ctx.shadowColor = COLORS.ENEMY_BULLET;
      this.ctx.beginPath(); this.ctx.arc(eb.x, eb.y, eb.radius, 0, Math.PI * 2); this.ctx.fill();
    ...
    this.bullets.forEach(b => {
      if (b.isBlackHole) {
        ...
        bhGrad.addColorStop(0, '#000'); bhGrad.addColorStop(0.6, '#4b0082'); bhGrad.addColorStop(1, 'transparent');
        this.ctx.fillStyle = bhGrad; this.ctx.beginPath(); this.ctx.arc(b.x, b.y, b.radius * 2, 0, Math.PI * 2); this.ctx.fill();
        this.ctx.fillStyle = '#000'; this.ctx.beginPath(); this.ctx.arc(b.x, b.y, b.radius, 0, Math.PI * 2); this.ctx.fill();
        this.ctx.strokeStyle = '#bf00ff'; this.ctx.lineWidth = 3;
        this.ctx.beginPath(); this.ctx.arc(b.x, b.y, b.radius + Math.sin(this.frameCount*0.2)*5, 0, Math.PI * 2); this.ctx.stroke();
      } else {
        this.ctx.lineWidth = b.radius * 2; this.ctx.lineCap = 'round'; this.ctx.strokeStyle = `rgba(0, 255, 0, 0.4)`;
        this.ctx.beginPath(); b.trail.forEach((pos, idx) => { if (idx === 0) this.ctx.moveTo(pos.x, pos.y); else this.ctx.lineTo(pos.x, pos.y); });
        this.ctx.stroke();
        this.ctx.fillStyle = COLORS.BULLET; this.ctx.beginPath(); this.ctx.arc(b.x, b.y, b.radius, 0, Math.PI * 2); this.ctx.fill();
      }
    });

    this.enemies.forEach(e => {
      const x = e.x + e.shakeX; const y = e.y + e.shakeY; const arch = ENEMY_ARCHETYPES[e.type];
      this.ctx.fillStyle = e.hitFlash > 0.45 ? 'white' : e.color;
      if (e.type === EnemyType.BOSS) { this.ctx.shadowBlur = 15; this.ctx.shadowColor = e.color; }
      this.ctx.beginPath(); const s = arch.sides;
      for (let i = 0; i < s; i++) {
        const a = (Math.PI * 2 / s) * i; const px = x + e.size * Math.cos(a); const py = y + e.size * Math.sin(a);
        if (i === 0) this.ctx.moveTo(px, py); else this.ctx.lineTo(px, py);
      }
      this.ctx.closePath(); this.ctx.fill(); this.ctx.shadowBlur = 0;
      const barW = e.size * 1.5; const barH = 5;
      this.ctx.fillStyle = 'rgba(0,0,0,0.6)'; this.ctx.fillRect(x - barW/2, y - e.size - 15, barW, barH);
      const hpRatio = Math.max(0, e.hp / e.maxHp);
      this.ctx.fillStyle = e.color; this.ctx.fillRect(x - barW/2, y - e.size - 15, barW * hpRatio, barH);
      if (e.type !== EnemyType.BOSS) {
        this.ctx.fillStyle = 'white'; this.ctx.font = '900 18px Monospace'; this.ctx.textAlign = 'center'; this.ctx.fillText(BalanceController.formatValue(e.hp), x, y + 6);
      }
    });

    this.ctx.textAlign = 'center';
    this.damageNumbers.forEach(dn => {
      this.ctx.globalAlpha = dn.life / dn.maxLife; this.ctx.font = dn.isCrit ? '900 32px Monospace' : '800 20px Monospace';
      this.ctx.fillStyle = dn.color; this.ctx.fillText(dn.text, dn.x, dn.y);
    });
    ...
    this.ctx.shadowBlur = 20; this.ctx.shadowColor = pc;
    ...
    this.ctx.fillStyle = 'white'; this.ctx.beginPath(); this.ctx.arc(this.player.x, this.player.y, this.player.radius * 0.45, 0, Math.PI * 2); this.ctx.fill();
    this.ctx.shadowBlur = 0;
    this.ctx.globalAlpha = 1.0;

    if (this.mercyBoost > 0) {
      this.ctx.fillStyle = '#ff0000'; this.ctx.font = 'bold 12px Orbitron'; this.ctx.textAlign = 'center';
